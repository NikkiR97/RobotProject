...
def autonomousPath(ultrasonicFD, ultrasonicRD, ultrasonicLD,
                          ultrasonicBD, orientation):
    generateMapFrom1Point4Block(positionX, positionY, ultrasonicFD,
                                ultrasonicRD, ultrasonicLD, ultrasonicBD,
                                orientation)
    nextLoc = getNextLoc(positionX, positionY)
    nextStep = nextStep((positionX, positionY), nextLoc, orientation)
    if nextStep == 1:
        if ultrasonicFD > 30:
            forward()
        else:
            spontaneousAutonomous1ultrasonic(ultrasonicFD)
    elif nextStep == 2:
        rotate_right()
        if ultrasonicFD > 30:
            forward()
